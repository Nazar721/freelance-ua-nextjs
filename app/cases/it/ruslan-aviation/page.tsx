...
export default function RuslanAviationPage() {
  ...
  const allImages = components.flatMap((c) => c.images || (c.image ? [c.image] : []));
    ...
    <article className="min-h-screen">
      ...
      <section ref={heroRef} className="py-8 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mt-6">
            <a
              href="https://www.ruslanaviationcomponents.com/"
              target="_blank"
            ...
            {components.map((comp, i) => (
              <FadeIn key={i} delay={0.1 + i * 0.08} y={30} blur={4}>
                <div className="featured-case-card border border-border rounded-2xl p-6 md:p-8" style={{ borderRadius: "16px" }}>
                  ...
                  {comp.image && (
                    <div className="mt-4 rounded-xl overflow-hidden border border-border cursor-pointer" onClick={() => openModal(allImages, allImages.indexOf(comp.image!), t(comp.titleKey))}>
                      <GalleryImage
                        src={comp.image}
                        alt={t(comp.titleKey)}
                        width={1200}
                  ...
                  {comp.images && (
                    <div className="mt-4 space-y-4">
                      {comp.images.map((img: string, idx: number) => (
                        <div key={idx} className="rounded-xl overflow-hidden border border-border cursor-pointer" onClick={() => openModal(comp.images!, idx, t(comp.titleKey))}>
                          <GalleryImage
                            src={img}
                            ...
                            height={800}
                            className="w-full h-auto object-contain"
                            onOpen={() => openModal(comp.images!, idx, t(comp.titleKey))}
                          />
                        </div>
                      ))}
                    </div>
        ...
        <div className="max-w-3xl mx-auto text-center">
          <FadeIn y={30} blur={8}>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="magnetic-button inline-flex items-center gap-2 border border-border text-purple-300 hover:border-purple-400/50 hover:bg-purple-400/10 hover:text-purple-200 font-semibold px-8 py-4 rounded-full transition-all duration-500-0.5 hover:shadow-[0_0_40px_rgba(167,139,250,0.12)]"
            >
